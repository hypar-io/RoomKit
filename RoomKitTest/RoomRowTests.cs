...
namespace RoomKitTest
{
    public class RoomRowTests
    {
        ...
        public void AddRoom()
        {
            ...
            {
                roomRow.AddRoom(room, false);
            }
            //Assert.Equal(18, roomRow.Rooms.Count);
            var model = new Model();
            foreach (Room room in roomRow.Rooms)
            {
                model.AddElement(new Space(new Profile(room.Perimeter), room.Height, new Material(room.Color, 0.0, 0.0, Guid.NewGuid(), Guid.NewGuid().ToString())));
            }
            model.AddElement(new Space(new Profile(polygon), 1.0, new Material(Colors.Aqua, 0.0, 0.0, Guid.NewGuid(), Guid.NewGuid().ToString())));
